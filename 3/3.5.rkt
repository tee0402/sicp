...
; map and stream-map require that the lengths of all argument lists or streams be the same
...
;  (define (update m num)
;    (cond ((eq? m 'generate) (rand-update num))
;          ((number? m) m)
;          (else (error "Unknown message: UPDATE" m))))
;  (define random-numbers
;    (cons-stream
;     random-init
;     (stream-map update request-stream random-numbers)))
;  random-numbers)
...
(define (random-in-range low high)
  (let ((range (- high low)))
    (+ low (random range))))
(define (integral-test-stream P x1 x2 y1 y2)
  (cons-stream
   (P (random-in-range x1 x2) (random-in-range y1 y2))
   (integral-test-stream P x1 x2 y1 y2)))
(define (area x1 x2 y1 y2)
  (* (- x2 x1) (- y2 y1)))
(define (estimate-integral P x1 x2 y1 y2)
  (let ((a (area x1 x2 y1 y2)))
    (stream-map (lambda (p) (* p a)) (monte-carlo (integral-test-stream P x1 x2 y1 y2) 0 0))))
(define (unit-circle x y)
  (<= (+ (square x) (square y)) 1))
;(stream-ref (estimate-integral unit-circle -1.0 1.0 -1.0 1.0) 100000)